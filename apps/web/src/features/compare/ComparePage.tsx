...
export const ComparePage: React.FC<ComparePageProps> = ({
  ...
  if (properties.length === 0) {
    return (
      <div className="bg-white dark:bg-slate-900/90 p-12 rounded-2xl border border-slate-200 dark:border-slate-800 text-center max-w-md mx-auto shadow-sm space-y-4">
        <Scale className="w-10 h-10 text-slate-300 mx-auto" />
        <h2 className="text-base font-bold text-slate-900 dark:text-white">No properties selected for comparison</h2>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button variant="outline" size="sm" onClick={onClearCompare}>
          <Trash2 className="w-3.5 h-3.5 mr-1 text-slate-500 dark:text-slate-400" />
          <span>Clear comparison ({properties.length})</span>
        </Button>
      </div>

      {/* Comparison Matrix Table */}
      <div className="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200 dark:border-slate-800 overflow-x-auto shadow-sm">
        <table className="w-full text-xs text-left border-collapse">
          <thead>
            <tr className="border-b border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/80/70">
              <th className="p-4 font-bold text-slate-700 dark:text-slate-200 w-44">Attribute</th>
              {properties.map((p) => (
                <th key={p.id} className="p-4 min-w-[220px] max-w-[260px] align-top">
                  ...
                  <div className="text-[11px] text-slate-500 dark:text-slate-400">{p.district}, {p.city}</div>
                </th>
              ))}
            </tr>
          </thead>
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {/* Asking Price */}
            <tr>
              <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">Asking Price</td>
              {properties.map((p) => {
            ...
            {/* Living Area */}
            <tr>
              <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">Living Area (m²)</td>
              {properties.map((p) => {
            ...
            {/* €/m² */}
            <tr>
              <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">Price per m²</td>
              {properties.map((p) => {
            ...
            {/* Rooms */}
            <tr>
              <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">Rooms & Layout</td>
              {properties.map((p) => (
            ...
            {/* Property Type */}
            <tr>
              <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">Property Type</td>
              {properties.map((p) => (
                <td key={p.id} className="p-4 text-slate-800">
                  {p.propertyType}
                </td>
            ...
            {/* Build Year */}
            <tr>
              <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">Build Year</td>
              {properties.map((p) => (
            ...
            <tr>
              <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">Maintenance Fee (Hoitovastike)</td>
              {properties.map((p) => (
                <td key={p.id} className="p-4 text-slate-800">
            ...
            {/* Balcony */}
            <tr>
              <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">Balcony (Parveke)</td>
              {properties.map((p) => (
                <td key={p.id} className="p-4">
                  {p.hasBalcony ? (
            ...
            {/* Sauna */}
            <tr>
              <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">Private Sauna (Oma sauna)</td>
              {properties.map((p) => (
                <td key={p.id} className="p-4">
                  {p.hasSauna ? (
                    <span className="text-emerald-600 font-bold flex items-center gap-1">
            ...
            {/* Elevator */}
            <tr>
              <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">Elevator (Hissi)</td>
              {properties.map((p) => (
                <td key={p.id} className="p-4">
                  {p.hasElevator ? (
                    <span className="text-emerald-600 font-bold flex items-center gap-1">
            ...
            {/* Energy Class */}
            <tr>
              <td className="p-4 font-semibold text-slate-700 dark:text-slate-200">Energy Class</td>
              {properties.map((p) => (
