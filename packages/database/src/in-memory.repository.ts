...
export class InMemoryPropertyRepository implements PropertyRepository {
  ...
  public seedDefaults() {
    ...
    if (favProp) {
      this.favorites.set('fav-user-demo-01-prop-hel-kallio-01', {
        id: 'fav-user-demo-01-prop-hel-kallio-01',
        userId: 'user-demo-01',
  ...
  public async getPropertyByExternalId(provider: string, externalId: string): Promise<Property | null> {
    for (const prop of this.properties.values()) {
      if (prop.provider === provider && prop.externalId === externalId) {
        return prop;
      }
    }
    return null;
  ...
  public async deleteSavedSearch(id: string, _userId?: string): Promise<boolean> {
  ...
  public async markNotificationAsRead(id: string, _userId?: string): Promise<boolean> {
